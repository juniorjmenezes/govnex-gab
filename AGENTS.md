...
- Consulte `docs/INTEGRACAO_GOVNEX_HUB.md` antes de mexer em usuários, vínculos ou papéis: a gestão deles está migrando para o Govnex Hub.
...
- Modais e painéis sobrepostos usam exclusivamente os primitivos compartilhados `Dialog`, `AlertDialog`, `Sheet` e `Drawer` de `components/ui`; não monte backdrop, portal, posicionamento, botão de fechar, título ou descrição manualmente. Preserve os títulos em `text-sm` sem caixa alta e os tokens de fundo, anel, sombra e animação definidos nesses componentes. Para dialogs com conteúdo longo, use `ScrollableDialogContent`, `ScrollableDialogHeader`, `ScrollableDialogBody` e `ScrollableDialogFooter` de `components/common/scrollable-dialog.tsx`. Exceções visuais devem ser resolvidas por variantes no componente compartilhado, não por uma implementação paralela na tela de negócio.
- Confirmações destrutivas simples usam `DestructiveAlertDialog` (`components/common/destructive-alert-dialog.tsx`), com ícone destrutivo, ação principal sólida, bloqueio durante o envio e fechamento somente após sucesso. Não recrie confirmações de exclusão combinando manualmente `Dialog` e botões. Toda confirmação identifica o registro afetado pelas props `subject` (nome, título) e `subjectDetail` (contexto curto, como data ou e-mail); em `DeleteRecordButton` o `subject` é obrigatório. Fluxos que exigem senha, código ou outros campos podem usar um dialog especializado, preservando a mesma hierarquia visual.
