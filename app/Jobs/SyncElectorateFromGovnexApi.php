...
/**
 * Sincroniza o eleitorado a partir da GOVNEX API — sem arquivo nenhum
 * envolvido, ao contrário de ProcessUploadedTseDataset/
 * DownloadAndProcessTseDataset (ver TsePoliticalDataSyncService::
 * GOVNEX_API_DATASETS).
 */
class SyncElectorateFromGovnexApi implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(
        ...
        $service->syncElectorateFromGovnexApi($run);
    }
    ...
    public function failed(?Throwable $exception): void
    {
        SincronizacaoTse::query()
            ...
            ->update([
                'situacao' => 'falhou',
                'erro' => Str::limit(
                    $exception?->getMessage() ?? 'Falha inesperada ao sincronizar o eleitorado pela GOVNEX API.',
                    10000,
                ),
                'concluida_em' => now(),
            ]);

        Log::warning('Falha ao sincronizar o eleitorado pela GOVNEX API.', [
