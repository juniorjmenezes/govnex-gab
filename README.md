...
Os dados oficiais do TSE — municípios, candidaturas, comparecimento, votação nominal, locais de votação, votação por seção e registro de pesquisas — são importados preferencialmente por upload manual em `/admin/gabinetes`: selecione o dataset, informe o ano e, quando necessário, a UF; abra o link oficial exibido, baixe o ZIP no navegador e envie-o para processamento na fila `tse`. A relação Município TSE/IBGE é uma base permanente, não solicita ano e deve ser importada apenas uma vez, salvo quando o TSE publicar uma nova versão; os demais conjuntos são separados por ano ou eleição. Como fallback explícito, a mesma tela permite tentar o download automático do ZIP oficial, que pode ser bloqueado pelo TSE com HTTP 403; não há mirror, cache ou agendamento automático. `TSE_CDN_URL` define a base oficial, `TSE_USER_AGENT`, `TSE_CONNECT_TIMEOUT`, `TSE_TIMEOUT` e `TSE_MAX_DOWNLOAD_MEGABYTES` controlam a tentativa automática, e `TSE_MANUAL_UPLOAD_MAX_MEGABYTES` limita o upload a 1024 MB por padrão. O conteúdo descompactado, a taxa de compressão e a quantidade de entradas também possuem limites próprios (`TSE_MAX_UNCOMPRESSED_MEGABYTES`, `TSE_MAX_COMPRESSION_RATIO` e `TSE_MAX_ARCHIVE_ENTRIES`). A sincronização PollingData permanece separada na tela política de cada gabinete.

O eleitorado é a exceção: em vez do ZIP oficial do TSE, ele é consumido da [GOVNEX API](https://github.com/juniorjmenezes/govnex-api), aplicação companheira que centraliza datasets públicos e publica um dataset "Perfil eleitorado" por UF. O mesmo painel oferece um card dedicado (`Eleitorado — GOVNEX API`) que dispara a sincronização informando só o ano, sem upload de arquivo; uma UF só entra quando a GOVNEX API já tiver o dataset daquela UF com `metadata.uf` marcado e o último import concluído, então a cobertura cresce aos poucos conforme mais estados são publicados lá, sem exigir o Brasil inteiro de uma vez. A plausibilidade de cada UF é validada contra a base local de municípios (`MunicipioEleitoral`) já importada — não mais contra uma faixa fixa nacional — e uma linha cuja UF não bate com o dataset declarado derruba a sincronização. `GOVNEX_API_URL` e `GOVNEX_API_KEY` configuram o acesso a essa API; sem a key, a sincronização ainda funciona, só com um limite de requisições mais baixo.
